...
private:
    ...
    inline Stack& operand_stack() { return m_current_stack_frame->operand_stack; }

    uint8_t bytecode_fetch_byte(uint8_t* code, size_t bytecode_size, size_t ptr);
    int16_t bytecode_fetch_short(uint8_t* code, size_t bytecode_size, size_t ptr);
