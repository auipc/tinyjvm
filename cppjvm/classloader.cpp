...
void ClassLoader::read_attributes(char* utf8, size_t utf8_length, uint16_t count) {
	for (int i = 0; i < count; i++) {
		...
		//m_stream->read_length(reinterpret_cast<void*>(info), attribute_length);
		if (strncmp(cp_entry.utf8, "Code", cp_entry.utf8_length) == 0) {
			...
			method_code[std::string(utf8, utf8_length)] = MethodCode{max_stack, max_locals, code_length, code};
			std::cout << "lel: " << std::string(utf8, utf8_length) << "\n";

			uint16_t exception_table_length = m_stream->read<uint16_t>();
			if (exception_table_length > 0) {
				throw std::runtime_error("exception_table_length > 0. we don't know how to parse exception_table_length!");
			}

			uint16_t attributes_count = m_stream->read<uint16_t>();
			read_attributes(utf8, utf8_length, attributes_count);
		} else if (strncmp(cp_entry.utf8, "LineNumberTable", cp_entry.utf8_length) == 0) {
		...
		} else if (strncmp(cp_entry.utf8, "ConstantValue", cp_entry.utf8_length) == 0) {
            uint16_t constantvalue_index = m_stream->read<uint16_t>();
        } else if (strncmp(cp_entry.utf8, "StackMapTable", cp_entry.utf8_length) == 0) {
uint16_t number_of_entries = m_stream->read<uint16_t>();
            for (int j = 0; j < number_of_entries; j++) {
                ...
                if (frame_type >= 0 && frame_type <= 63) {
                    ...
                    throw std::runtime_error("same_locals_1_stack_item_frame not implemented");
                } else if (frame_type == 247) {
                    // same_locals_1_stack_item_frame_extended
                    throw std::runtime_error("same_locals_1_stack_item_frame_extended not implemented");
                } else if (frame_type >= 248 && frame_type <= 250) {
                    // chop_frame
                    uint16_t offset_delta = m_stream->read<uint16_t>();
                } else if (frame_type == 251) {
                    // same_frame_extended
                    throw std::runtime_error("same_frame_extended not implemented");
...
void ClassLoader::load_class() {
			...
			case ConstPoolTag::Utf8:
				{
					...
					// This isn't terminated by a NUL byte so be careful when operating on it.
					char* buffer = new char[length];
					m_stream->read_length(reinterpret_cast<void*>(buffer), length);
					constant_pool.push_back(ConstPoolEntry{ConstPoolTag::Utf8, buffer, length});
	...
	if (interfaces_count > 0) {
		...
		m_stream->read_length(reinterpret_cast<void*>(interfaces), interfaces_count * sizeof(uint16_t));
		delete[] interfaces;
	}
	...
	if (fields_count > 0) {
        for (int i = 0; i < fields_count; i++) {
            ...
            if (attributes_count > 0) {
                ...
                read_attributes(cp_entry.utf8, cp_entry.utf8_length, attributes_count);
            }
        }
	}
    ...
    printf("\033[0;31mFIXME: buggy stack map table parsing so we can't read attribute count\033[0m\n");
	//uint16_t attributes_count = m_stream->read<uint16_t>();
