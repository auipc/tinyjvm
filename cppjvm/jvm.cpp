...
JVM::JVM(ClassLoader* classloader)
...
// TODO allocate memory for attributes of the class and figure out how to resolve dependencies in a clean way
// so we can call Object's initializer in case there's implementation specific quirks in the standard java lib
// Maybe we should write our own :^)
void JVM::run() {
	auto code = m_classloader->method_code[std::string("main")];
	printf("A %d\n", code.code_length);
    do {
        uint8_t opcode = bytecode_fetch_byte(code.code, code.code_length, m_pc++);
        interpret_opcode(opcode);
        if ((m_pc+1) > code.code_length) m_exit = true;
    } while (!m_exit);
}

uint8_t JVM::bytecode_fetch_byte(uint8_t* code, size_t bytecode_size, size_t ptr) {
    assert(ptr < bytecode_size);
    return code[ptr];
}

int16_t JVM::bytecode_fetch_short(uint8_t* code, size_t bytecode_size, size_t ptr) {
    assert(ptr < bytecode_size);
...
void JVM::interpret_opcode(uint8_t opcode) {
    ...
    case 0x36:
    {
        uint8_t index = bytecode_fetch_byte(m_classloader->method_code[std::string("main")].code, m_classloader->method_code[std::string("main")].code_length, m_pc++);
        /*if (index > 10) {
            throw std::runtime_error("Variable index out of bounds");
        }
    ...
    case 0x84:
    {
        ...
        int8_t constant = bytecode_fetch_byte(m_classloader->method_code[std::string("main")].code, m_classloader->method_code[std::string("main")].code_length, m_pc++);
    ...
    case 0xa7:
    {
        int16_t offset = bytecode_fetch_short(m_classloader->method_code[std::string("main")].code, m_classloader->method_code[std::string("main")].code_length, m_pc);
    ...
    case 0x10:
    {
        uint8_t value = bytecode_fetch_byte(m_classloader->method_code[std::string("main")].code, m_classloader->method_code[std::string("main")].code_length, m_pc++);
